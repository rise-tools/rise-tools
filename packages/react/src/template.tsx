...
export function isActionEvent(
  event: TemplateEvent,
): event is TemplateEvent<ActionEventDataState> {
...
export type ActionEvent<T = any, K = any> = TemplateEvent<
  ActionEventDataState<T>,
  K
>;

export function isCompositeDataState(
  obj: any,
): obj is ComponentDataState | ReferencedDataState {
  return (
    obj !== null &&
    typeof obj === "object" &&
    "$" in obj &&
...
export function isComponentDataState(
  obj: JSONValue,
): obj is ComponentDataState {
  return (
    obj !== null &&
    typeof obj === "object" &&
    "$" in obj &&
...
export function BaseTemplate({
  ...
  function renderComponent(stateNode: ComponentDataState, path: Path) {
    ...
    const children = stateNode.children
      ? render(stateNode.children, path)
      : null;

    return (
      <Component key={stateNode.key} {...componentProps} children={children} />
    );
  }
  ...
  function render(stateNode: JSONValue, path: Path): React.ReactNode {
    ...
    if (stateNode.$ === "ref") {
      throw new Error(
        "Your data includes refs. You must use a <Template /> component instead.",
      );
    }
  }
  ...
  function renderProp(
    ...
    if (isEventDataState(propValue)) {
      return async (payload: any) => {
        ...
        if (Array.isArray(propValue)) {
          propValue.forEach((eventPropItem) => {
            if (
              typeof eventPropItem === "object" &&
              eventPropItem?.["$"] === "event"
            ) {
              return onEvent?.({
                target,
                dataState: eventPropItem,
                payload,
              });
            }
          });
    ...
    if (Array.isArray(propValue)) {
      return propValue.map((item) =>
        renderProp(propKey, item, parentNode, path),
      );
    }
